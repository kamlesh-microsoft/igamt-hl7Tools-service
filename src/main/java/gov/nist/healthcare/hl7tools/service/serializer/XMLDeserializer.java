...
import static gov.nist.healthcare.hl7tools.service.serializer.XMLUtils.*;
...
public class XMLDeserializer {
	...
	public Message deserialize(InputStream xml, ProfileSchemaVersion schemaVersion) throws Exception {
		if( schemaVersion != ProfileSchemaVersion.V29 ) throw new Exception("Deserialization for Profile Schema prior to version 2.9 is not yet implemented.");
	...
	private List<gov.nist.healthcare.hl7tools.domain.Element> processElements(Elements children, ProfileSchemaVersion schemaVersion) {
		List<gov.nist.healthcare.hl7tools.domain.Element> result = new ArrayList<gov.nist.healthcare.hl7tools.domain.Element>();
		int position = 1;
	...
	private gov.nist.healthcare.hl7tools.domain.Element processElement(Element e, int position, ProfileSchemaVersion schemaVersion) {
		gov.nist.healthcare.hl7tools.domain.Element ee = new gov.nist.healthcare.hl7tools.domain.Element();
		...
		ee.setConformanceStatementList( processConformanceStatements(e.getChildElements("ConformanceStatement")) );
		Usage trueUsage =  isSchemaPriorToV29(schemaVersion) ? Usage.valueOf( e.getAttributeValue("PredicateTrueUsage") ) : null;
		Usage falseUsage = isSchemaPriorToV29(schemaVersion) ? Usage.valueOf( e.getAttributeValue("PredicateFalseUsage") ) : null;
		ee.setPredicate( processPredicate(e.getFirstChildElement("Predicate"), trueUsage, falseUsage, schemaVersion) );
	...
	private Field processField(Element e, int position, ProfileSchemaVersion schemaVersion) {
		...
		f.setConformanceStatementList( processConformanceStatements(e.getChildElements("ConformanceStatement")) );
		Usage trueUsage =  isSchemaPriorToV29(schemaVersion) ? Usage.valueOf( e.getAttributeValue("PredicateTrueUsage") ) : null;
		Usage falseUsage = isSchemaPriorToV29(schemaVersion) ? Usage.valueOf( e.getAttributeValue("PredicateFalseUsage") ) : null;
		f.setPredicate( processPredicate(e.getFirstChildElement("Predicate"), trueUsage, falseUsage, schemaVersion) );
		return f;
	}
	...
	private Component processComponent(Element e, int position, ProfileSchemaVersion schemaVersion) {
		...
		c.setConformanceStatementList( processConformanceStatements(e.getChildElements("ConformanceStatement")) );
		Usage trueUsage =  isSchemaPriorToV29(schemaVersion) ? Usage.valueOf( e.getAttributeValue("PredicateTrueUsage") ) : null;
		Usage falseUsage = isSchemaPriorToV29(schemaVersion) ? Usage.valueOf( e.getAttributeValue("PredicateFalseUsage") ) : null;
		c.setPredicate( processPredicate(e.getFirstChildElement("Predicate"), trueUsage, falseUsage, schemaVersion) );
		return c;
	}
	
	private Predicate processPredicate(Element predicate, Usage trueUsage, Usage falseUsage, ProfileSchemaVersion schemaVersion) {
		if( predicate == null ) return null;
		Predicate p = new Predicate();
		p.setDescription( predicate.getFirstChildElement("Description").getValue() );
		p.setTrueUsage( trueUsage  != null ? trueUsage  : Usage.valueOf( predicate.getAttributeValue("PredicateTrueUsage") ));
		p.setFalseUsage(falseUsage != null ? falseUsage : Usage.valueOf( predicate.getAttributeValue("PredicateFalseUsage") ));
	...
	private Map<String, Datatype> buildPartialDatatypeLibrary(Element staticDef, ProfileSchemaVersion schemaVersion) {
	...
	private void updateDatatypeComponents(Map<String, Datatype> dtLib, Element staticDef, ProfileSchemaVersion schemaVersion) {
		if( schemaVersion == ProfileSchemaVersion.V29 ) {
			Elements datatypes = staticDef.getFirstChildElement("Datatypes").getChildElements("Datatype");
			for(int i = 0; i < datatypes.size(); i++) {
	...
	private Document build(InputStream xml, ProfileSchemaVersion schemaVersion) throws Exception {
		try {
			...
			throw new Exception("[Error] An error occurs while parsing or validating the XML document. " + e.getMessage(), e.getCause()) ;
		}
	}
	
	// SAX REader configuration
	private XMLReader createSAXReader(ProfileSchemaVersion schemaVersion) throws SAXException, ParserConfigurationException {
