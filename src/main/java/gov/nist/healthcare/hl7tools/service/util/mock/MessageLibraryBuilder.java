	...
	private static List<Element> buildGroups(SegmentLibrary segmentLibrary,
		...
		for (gov.nist.healthcare.hl7tools.service.util.mock.hl7.domain.Group g : map.values()) {
			...
			if (g.getChildren() != null) {// Some messages in old versions don't
											// have children
				log.info("o loop=" + " gid=" + g.getId() + " g.name=" + g.getName() + " size=" + g.getChildren().size());
	...
	private static Element updateChildren(gov.nist.healthcare.hl7tools.service.util.mock.hl7.domain.Element ee,
		...
		if (ee.getSegmentId() != null) {
			...
			if (map.get(ee.getGroupId()).getChildren() != null) {
				log.info("i loop"  + " gid=" + map.get(ee.getGroupId()).getId() + " g.name=" + map.get(ee.getGroupId()).getName() + " size="+ map.get(ee.getGroupId()).getChildren().size());
			} else {
				log.info("no loop");
			}
		...
		log.info("recur=" + recur + " ee.id=" + ee.getId() + " ee.segId=" + ee.getSegmentId() + " parentId=" + ee.getParentId() + "parentName=" + map.get(ee.getParentId()).getName());
		recur--;
		return e;
	}
